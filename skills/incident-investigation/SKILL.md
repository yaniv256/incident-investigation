...
10. **If you are an AI agent: you are not human — suspect inherited-affordance mismatches.** You are a next-token predictor trained on human behavior, so you inherit priors that assume human affordances you do not actually have. When your own behavior looks embarrassingly stupid in hindsight — you never took a screenshot; you trusted a projection you wrote but never tested; you concluded the *task* was impossible rather than your *check* was broken — suspect this class of error first. It is a recurring, high-value application of the maximum-pain principle (#9), because "I mimicked a human capability I lack" is precisely the kind of self-implicating explanation the mind routes around. The flagship instance is visual: you act as if you can see the rendered page, but you are blind without a screenshot (Default Action 4). The same shape appears elsewhere — e.g. pricing your own effort like an expensive human's when your marginal cost of an extra check, an extra query, or a line of code is near zero, and so skipping cheap verification "to save effort." Ask explicitly: *what human affordance am I unconsciously assuming that I don't have here?* Then supply it deliberately — take the screenshot, run the check, add the instrumentation.
