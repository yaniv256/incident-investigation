...
**4. Look at it — take a screenshot, then validate the instrument.** If the incident involves rendered output, look before trusting a DOM read, projection, or tool success flag. An AI agent systematically under-uses visual evidence because the humans in its training data could see continuously; the agent cannot. But a screenshot is **not automatically ground truth**. It is an asymmetric measurement: a newly visible change is strong positive evidence, while “nothing changed” may be a stale-frame false negative when a display is dormant, a tab is occluded, or a canvas compositor did not repaint. Before using pixels as clearance, establish `surface_identity`, inspect the screenshot's `freshness`, and prove the instrument can show a known current marker. If freshness is unverified, triangulate with an independent model/export/state projection. For Docs/Sheets/Slides and other canvas apps, semantic state comes from the application model; screenshots confirm layout/appearance only. Never trust a projection you authored without independent evidence, and never use an uncalibrated screenshot as that evidence merely because it is visual.
...
11. **The true cause is usually NOT in your first-pass list — experiments generate hypotheses, they don't just referee them.** Plausible explanations are effectively unbounded, and the space of possible causes is far larger than the handful you can enumerate before touching the system. So your first-cut list of 5-8 hypotheses probably does *not* contain the real one — a reasonable working prior is that the correct hypothesis is *accessible before experimentation only ~20% of the time*. **There is a structural reason the number is this low, not just bad luck: on a system you built, every failure mode you could imagine, you already defended against while building it.** The concerns within your imagination were dealt with in the code — you handled the null, guarded the race you foresaw, validated the input you pictured. So the bugs that actually survive to become incidents are, almost *by construction*, the ones you did **not** imagine — the failure modes outside your generative reach. Your pre-experiment hypothesis list is assembled from exactly the same imagination that already eliminated everything it could think of; that is why the list so reliably misses. The real cause lives in your blind spot by definition, and only contact with the data — not more thinking from the same head that built the system — can drag it into view. The consequence reframes what an experiment is *for*: it is not merely a referee that picks the winner among your named hypotheses — it is the instrument that *manufactures* the hypothesis you could not think of, by putting you in contact with data your imagination did not predict. Therefore: (a) hold your whole initial list at a low prior and reserve real probability mass for "none of the above" (see the mandatory unlisted-cause row in Phase 2); (b) when your listed hypotheses start dying one after another, that is **not** failure to be mourned — it is the signal to go get *more data*, because the truth is out past your current imagination and only fresh observation reaches it; (c) prefer experiments that could *reveal a new cause* (drive the real thing yourself, strip out a layer, measure an intermediate value) over experiments that only discriminate among the named few. Calibrate against your own record: grep your past investigation files for CONFIRMED vs REFUTED per hypothesis — the ratio is your measured first-pass hit-rate, and it is far lower than you feel. Not "under half" — that phrasing is itself the fabricated comfort, still pretending the number lives *near* half. It does not, for anyone: no human on the planet gets close to half on a genuinely novel symptom, and neither will you. The honest range is a small minority — often single digits, rarely above ~20%. When you catch yourself softening the number toward "roughly even," that softening IS the motivated reasoning; write the brutal number down instead. **The felt-sense of a healthy investigation is surprise.** Because the true cause lives outside your imagination, the emotional signal that you are getting close is not "aha, confirmed" — it is *"wait, what? that's not what I expected."* Data that merely matches your prediction tells you almost nothing: it keeps you inside your own model, which is precisely where the cause is *not*. Data that surprises you is the sound of contact with the part of reality you could not predict — the blind spot cracking open. So treat surprise as the compass: chase the experiment that could astonish you, and when a result lands exactly as expected, distrust it — a run of unsurprising confirmations usually means you are spinning inside your imagination, not reaching past it. An investigation with no surprises has probably not found anything yet. **Make it a ritual: mandatory champagne every time you're shocked.** Mark the surprising result explicitly in the investigation file — a 🍾 beside it — and *stop to mine it* instead of rushing to explain it away. This is a deliberate counter-incentive: a surprise creates discomfort (it means your model was wrong), and the untrained reflex is to smooth it over and get back inside your comfortable story as fast as possible. Celebrating the shock inverts that reflex — the surprise becomes the prize, the moment to lean *in*. Every 🍾 is the investigation working; a file full of them is a healthy one. Toast the result that proves you wrong, because it is the one teaching you where the cause actually lives.

12. **Your hypothesis probabilities are entangled with your own mind — they can never be fully objective.** This is the deepest consequence of #11, and it changes how to read every number in the table. A hypothesis probability *looks* like a claim about the world outside you — "there's a 30% chance it's a thread-pool exhaustion." But the probabilities are normalized to sum to 1 over the set of hypotheses *you were able to generate*, so every P(Hᵢ) is silently conditioned on your own imagination: it is really P(Hᵢ | the true cause is something I could think of). And the leftover — the "none of the above" mass — is **not a fact about the world at all; it is a measurement of your own generative limitation.** It is a hypothesis about *you*: the probability that your imagination failed to contain reality. That means you cannot factor the observer out of the estimate. Any probability you assign to an external state of the world has already passed through, and been shaped by, your capacity to imagine that state — the events outside you and your ability to conceive them are entangled, and you are inside the system you are trying to measure. **So treat every probability as inescapably subjective — a statement about your current state of knowledge and imagination, never a God's-eye reading of the world.** Practically: (a) the "none of the above" row is the one honest place in the table where this shows up explicitly — it is your self-estimate of your own blindness, so give it the weight your track record demands (#11), not the small residue that feels tidy; (b) never mistake a confident, well-normalized distribution for an accurate one — internal coherence (the numbers summing neatly, each hypothesis crisply argued) is not contact with reality, and a beautifully calibrated table over the wrong hypothesis set is still wrong; (c) the only thing that loosens the entanglement is external data that can surprise you (#11's compass) — an experiment reaches outside your imagination in a way that re-estimating from the same head never can. Objectivity is not something you *reason* your way to from the inside; it is something you *earn* by letting the world push back.
...
**You cannot blame the user. "The user made a mistake" is a hypothesis-space attractor — redirect it to the interface.** When the failure involves a *human or agent operator* doing the wrong thing — routed to the wrong target, picked the wrong option, misread the state, "should have been more careful" — **do not stop at blaming the operator.** "The user is at fault" is the same *exculpatory attractor* as "it's the platform's fault" or "I'm blocked" (see Phase 2): it is attractive precisely because **if the user is to blame, nothing needs to be built — the pressure to find and fix a real defect ends.** That relief is the tell that it is disproportionately likely to be the wrong stopping point. The correction is a rule, not a case-by-case judgment: **a mistake the operator makes — especially the SAME mistake again and again — is evidence of a bad USER INTERFACE, and the interface is where the blame belongs.** In this project the operator is usually *you* (the agent), and your interface is the **MCP / tool / status / error surface** you operate through; a recurring "which runtime / which tab / which option" error is an interface defect (a status that lies, an opaque error code, a misleading field, no affordance for the obvious intent), not an operator-discipline failure. Assign Level-1/2/3 blame to that interface. Operator habits ("verify before trusting", "don't stop to ask") may appear as *secondary mitigations*, but they are the compensations a good interface would make unnecessary — never the root, because a design that requires the operator to compensate perfectly forever is the design that reproduces the error. When you catch yourself writing "the operator should have…", stop and ask "**what did the interface do that made that error easy, and what would have made it hard?**" — that question is the actual root cause.
